...
if($_POST['accept']){
	 ...
	 $invoiceno  = $_POST['invoiceno'];
									...
									$qry2 ="UPDATE `bill` SET `orderstatus`='2',`status`='4' WHERE `invoiceno`='$invoiceno'";
...
<body>
	...
	<?php	
	 $sql = "SELECT * FROM `bill` WHERE `refid`='".$uid."' AND `status` ='2'";
	...
	<form method="post" action="pendindelivary.php">
		<input type="hidden" name="invoiceno" value="<?php echo $invoiceno; ?>">
	<input type="hidden" name="amount" value="<?php echo $amount; ?>">
 	<input type="submit" class="btn btn-success" value="Delivary Completed" name="accept">
