...
if(isset($_SESSION["uid"]) && isset($_SESSION["role"])) 
{
	if($_SESSION["role"] == 1) 
	{
		...
		{
			$cat_id = $_GET['cat_id'];
			$del_cat_query = "UPDATE `productcategory` SET `status`=1 WHERE `cat_id`=".$cat_id."";
			$del_cat_query_run = $con->query($del_cat_query);
			if($del_cat_query_run)
			{
				$del_sub_cat_query = "UPDATE `productsubcategory` SET `status`=1 WHERE `cat_id`=".$cat_id."";
				$del_sub_cat_query_run = $con->query($del_sub_cat_query);
				if($del_sub_cat_query_run)
				{
					$success_delete = "You have Activate Catagory and Sub-Catagory Successfully";
				}
				else
				{
